 ...
 * ```
 *
 * When a client connects, the server expects a PB request message to be sent.
 * It will then check if credentials are currently known. If not, the socket
 * will stay open for a maximum of `options.timeout` while the server waits to
 * receive credentials. If the timeout exceeds without having seen credentials,
 * the connection is destroyed. Otherwise, a PB response message is sent.
 *
